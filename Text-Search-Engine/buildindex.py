...
class BuildIndex:
	...
	def process_files(self):
		...
		for key, value in self.filenames.items():
			...
			stemmed_words = [stemmer.stem(word).lower() for word in WordPunctTokenizer().tokenize(value)]
			filtered_word = []
			for word in stemmed_words:
				if len(word) > 1:
					filtered_word.append(word)
			file_to_terms[key] = [word for word in filtered_word if word not in english_stops]
